                ...
                dbc.Col(
                    [
                        ...
                        html.Div(
                            [
                                html.P(
                                    "Selecione que tipo de dado deseja visualizar:",
...
def display_status(date, location):
    ...
    recuperados_novos = (
        "-" if df_data_on_date["Recuperadosnovos"].isna().values[0] else
        f'{int(df_data_on_date["Recuperadosnovos"].values[0]):,}'.replace(
            ",", "."))
    acompanhamentos_novos = (
        "-" if df_data_on_date["emAcompanhamentoNovos"].isna().values[0] else
        f'{int(df_data_on_date["emAcompanhamentoNovos"].values[0]):,}'.replace(
            ",", "."))
    casos_acumulados = (
        "-" if df_data_on_date["casosAcumulado"].isna().values[0] else
    ...
    casos_novos = ("-" if df_data_on_date["casosNovos"].isna().values[0] else
                   f'{int(df_data_on_date["casosNovos"].values[0]):,}'.replace(
                       ",", "."))
    obitos_acumulado = (
        "-" if df_data_on_date["obitosAcumulado"].isna().values[0] else
        f'{int(df_data_on_date["obitosAcumulado"].values[0]):,}'.replace(
            ",", "."))
    obitos_novos = (
        "-" if df_data_on_date["obitosNovos"].isna().values[0] else
...
def plot_line_graph(plot_type, location):
    ...
    if plot_type in bar_plots:
        fig2.add_trace(
            go.Bar(x=df_data_on_location["data"],
                   y=df_data_on_location[plot_type]))
    else:
        fig2.add_trace(
            go.Scatter(x=df_data_on_location["data"],
                       y=df_data_on_location[plot_type]))
